...
import ReplicaHost3.DCRS.DCRSWrong;
...
public class Replica3 {
    ...
    public Replica3(Logger logger , DCRSImpl compServer, DCRSImpl soenServer, DCRSImpl inseServer, DCRSWrong compWrong, DCRSWrong soenWrong, DCRSWrong inseWrong){
        this.logger = logger;
    ...
    public static void main(String[] args) throws IOException {
        Logger compLogger = Logger.getLogger("comp.server3.log");
        configLogger("compServer3",compLogger);

        Logger soenLogger = Logger.getLogger("soen.server3.log");
        configLogger("soenServer3",soenLogger);

        Logger inseLogger = Logger.getLogger("inse.server3.log");
        configLogger("inseServer3",inseLogger);

        Logger replicaLogger = Logger.getLogger("replica3.log");
        configLogger("replica3", replicaLogger);
        ...
        Replica3 replica3 = new Replica3(replicaLogger , compServer , soenServer , inseServer, compWrong, soenWrong, inseWrong);

        Runnable compTask = () -> {
            try {
                replica3.startUdpServer(6667, compServer);
            } catch (Exception e) {
                e.printStackTrace();
            }
        };
        ...
        Runnable soenTask = () -> {
            try {
                replica3.startUdpServer(6668, soenServer);
            } catch (Exception e) {
                e.printStackTrace();
            }
        };
        ...
        Runnable inseTask = () -> {
            try {
                replica3.startUdpServer(6669, inseServer);
            } catch (Exception e) {
                e.printStackTrace();
            }
        };
